    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
        * @brief Type of mass transfer.
        */
        std::string massType() const
        {
            return config["physics"]["mass transfer"]["type"].as<std::string>();
        }

        /**
        * @brief Coefficients for first mass transfer.
        */
        T mass1alpha() const
        {
            return config["physics"]["mass transfer"]["coefficients1"][0].as<T>();
        }
        T mass1beta() const
        {
            return config["physics"]["mass transfer"]["coefficients1"][1].as<T>();
        }

        /**
        * @brief Coefficients for second mass transfer.
        */
        T mass2alpha() const
        {
            return config["physics"]["mass transfer"]["coefficients2"][0].as<T>();
        }
        T mass2beta() const
        {
            return config["physics"]["mass transfer"]["coefficients2"][1].as<T>();
        }
        ...
        */
        ...
        /**
        * @brief MassSnapshot file path for the given step.
        * @param step Current step
        */
        std::string masssnapshotPath(int step) const
        {
            auto path = config["output"]["snapshot"]["file"].as<std::string>();

            // insert mass and replace the * with the current step
            std::string str="mass_";
            path.insert(path.find("s"), str);
            path.replace(path.find("*"), 1, std::to_string(step));
