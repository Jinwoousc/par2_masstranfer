...
void run(int argc, char** argv)
{
    ...
    // std::string configoriginal = configurationFile;

    for (int i = 0; i < 1; i++)
    {
      // std::string casen = std::to_string(i);
      // configurationFile = configoriginal;
      // configurationFile += casen;
      // configurationFile += ".yaml";
      console->info("configurationFile {}", configurationFile);
      ...
      // Create and initialize mass field
      console->info("Initialize massfield...");
      thrust::host_vector<int> mmb1, mmb2, mimb1, mimb2;
      int size = grid.nx*grid.ny*grid.nz;
      mmb1.resize(size, 0);
      mmb2.resize(size, 0);
      mimb1.resize(size, 0);
      mimb2.resize(size, 0);
      // par2::massfield::build<int>(grid, mmb1, mmb2, mimb1, mimb2);

      int masstransfer;
      if (par.massType() == "one-site")
      {
          masstransfer = 1;
      }
      else if (par.massType() == "two-site")
      {
          masstransfer = 2;
      }
      else
      ...
      PAR2_FLOAT ddt = par.dt();
      PAR2_FLOAT mass1alpha = par.mass1alpha();
      PAR2_FLOAT mass1beta = par.mass1beta();
      PAR2_FLOAT expvalue1 = exp(-(1+mass1beta)*mass1alpha*ddt);

      PAR2_FLOAT P100 = ( 1 + mass1beta*expvalue1 ) / ( 1+mass1beta );
      PAR2_FLOAT P101 = ( 1 - expvalue1 ) / ( 1+mass1beta );
      PAR2_FLOAT P110 = ( mass1beta - mass1beta*expvalue1 ) / ( 1+mass1beta );
      PAR2_FLOAT P111 = ( mass1beta + expvalue1 ) / ( 1+mass1beta );

      logfile->info("P100: {}", P100);
      logfile->info("P101: {}", P101);
      logfile->info("P110: {}", P110);
      logfile->info("P111: {}", P111);

      par2::PParticles<PAR2_FLOAT> particles(grid, mmb1, mmb2, mimb1, mimb2, datax, datay, dataz,
                                             par.Dm(), par.alphaL(), par.alphaT(),
                                             masstransfer, par.mass1alpha(), par.mass1beta(), par.mass2alpha(), par.mass2beta(),
      ...
      for (auto step = 0; step < steps+1; step++)
      {
          ...
          if (step == stepSnapshot)
          {
              ...
              auto masssnapshotOutputPath = configurationPath + par.masssnapshotPath(step);

              logfile->info("Writing Snapshot file {} (STEP {})", snapshotOutputPath, step);
              console->info("Writing Particle Snapshot file {}", snapshotOutputPath);
              particles.exportCSV(snapshotOutputPath);

              console->info("Writing Mass Snapshot file {}", masssnapshotOutputPath);
              particles.massexportCSV(masssnapshotOutputPath);
